...
class TestDocMonitor:
    ...
    async def test_run_generates_reports_on_changes(
        ...
        # Note: Daily index is now generated by CLI (not monitor.run) for batch analysis support

    @patch("src.main.DocumentFetcher")
    async def test_run_ignores_nonce_only_changes(
        self,
        mock_fetcher_class: MagicMock,
        mock_source: SourceConfig,
        mock_config: Config,
        ...
        """Nonce-only changes in HTML should not trigger a diff."""
        mock_source.docs_dir.mkdir(parents=True)
        # Stored content already has nonce stripped (was normalized on save)
        (mock_source.docs_dir / "overview.md").write_text(
            '<link href="/s.css" /><div>Content</div>'
        )

        # Fetched content has a nonce attribute that should be stripped
        mock_fetcher = AsyncMock()
        mock_fetcher.fetch_all.return_value = [
            FetchResult(
                "overview",
                '<link href="/s.css" nonce="NEW_NONCE==" /><div>Content</div>',
                200,
            )
        ]
        mock_fetcher.__aenter__ = AsyncMock(return_value=mock_fetcher)
